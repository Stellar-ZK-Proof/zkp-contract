...
impl ZkpPrivatePay {
    ...
    pub fn submit_payment(
        ...
        input.append(&Bytes::from(commitment.clone()));
        for b in timestamp.to_be_bytes().iter() { input.push_back(*b); }
        let tx_id: BytesN<32> = BytesN::from(env.crypto().sha256(&input));
        let record = TxRecord {
            commitment, sender, timestamp,
            status: PaymentStatus::Pending,
            nullifier: nullifier.clone(), audit_ref_hash,
        };
...
mod tests {
    ...
    fn dummy(env: &Env, seed: u8) -> BytesN<32> {
        ...
        BytesN::from(env.crypto().sha256(&b))
    }
